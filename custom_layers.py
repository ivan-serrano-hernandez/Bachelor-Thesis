...
def cspdarknet53(input):
    # x = conv(input, 32, 3)
    # x = conv(x, 64, 3, downsampling=True)
    x = conv(input, 32, 3, activation='mish')
    x = conv(x, 64, 3, activation='mish', downsampling=True)
    ...
    x = layers.Concatenate()([layers.MaxPooling2D(pool_size=13, strides=1, padding='same')(x),
                              layers.MaxPooling2D(
                                  pool_size=9, strides=1, padding='same')(x),
                              layers.MaxPooling2D(
                                  pool_size=5, strides=1, padding='same')(x),
                              x
                              ])
    x = conv(x, 512, 1)
    x = conv(x, 1024, 3)
...
def yolov4_neck(x, num_classes):
    ...
    route0 = x
    x = conv(x, 256, 3)
    conv_sbbox = conv(x, 3 * (num_classes + 5), 1,
    ...
    route1 = x
    x = conv(x, 512, 3)
    conv_mbbox = conv(x, 3 * (num_classes + 5), 1,
                      activation=None, batch_norm=False)
    ...
    x = conv(x, 1024, 3)
    conv_lbbox = conv(x, 3 * (num_classes + 5), 1,
                      activation=None, batch_norm=False)
...
def nms(model_ouputs, input_shape, num_class, iou_threshold=0.413, score_threshold=0.3):
    ...
    for output_idx in range(0, len(model_ouputs), 4):
        ...
        class_probabilities = tf.concat(
            [class_probabilities, tf.reshape(output_classes, (bs, -1, num_class))], axis=1)
        # class_probabilities = K.print_tensor(class_probabilities, message='class_probabilities at nms = ', summarize=-1)
