...
def bbox_ciou(boxes1, boxes2):
    ...
    p2 = K.pow(boxes1[..., 0] - boxes2[..., 0], 2) + \
        K.pow(boxes1[..., 1] - boxes2[..., 1], 2)
...
def yolo_loss(args, num_classes, iou_loss_thresh, anchors):
    ...
    sbbox_ciou_loss, sbbox_conf_loss, sbbox_prob_loss = loss_layer(
        conv_sbbox, pred_sbbox, label_sbbox, true_bboxes, 8, num_classes, iou_loss_thresh)
    mbbox_ciou_loss, mbbox_conf_loss, mbbox_prob_loss = loss_layer(
        conv_mbbox, pred_mbbox, label_mbbox, true_bboxes, 16, num_classes, iou_loss_thresh)
    lbbox_ciou_loss, lbbox_conf_loss, lbbox_prob_loss = loss_layer(
        conv_lbbox, pred_lbbox, label_lbbox, true_bboxes, 32, num_classes, iou_loss_thresh)
    ...
    ciou_loss = K.print_tensor(ciou_loss, message='ciou_loss=')
    conf_loss = K.print_tensor(conf_loss, message='conf_loss=')
    prob_loss = K.print_tensor(prob_loss, message='prob_loss=')

    test_loss = ciou_loss+conf_loss+prob_loss
    test_loss = K.print_tensor(test_loss, message='test_loss=')
...
def loss_layer(conv, pred, label, bboxes, stride, num_class, iou_loss_thresh):
    ...
    # Coordinate loss
    ciou = tf.expand_dims(bbox_giou(pred_xywh, label_xywh),
                          axis=-1)  # (8, 13, 13, 3, 1)
    ...
    ciou_loss = respond_bbox * bbox_loss_scale * \
        (1 - ciou)  # iou loss for respond bbox
    ...
    expand_bboxes = bboxes[:, np.newaxis, np.newaxis,
                           np.newaxis, :, :]  # (?, 1, 1, 1, 70, 4)
...
def decode(conv_output, anchors, stride, num_class):
    ...
    conv_output = tf.reshape(
        conv_output, (batch_size, output_size, output_size, anchor_per_scale, 5 + num_class))
