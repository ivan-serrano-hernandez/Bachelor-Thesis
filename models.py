...
class Yolov4(object):
    def __init__(self,
        ...
        self.class_color = {name: list(np.random.random(
            size=3)*255) for name in self.class_names}
        # Training
        ...
        if yolo_config['num_gpu'] > 1:
            ...
            with mirrored_strategy.scope():
                self.build_model(
                    load_pretrained=True if self.weight_path else False)
        else:
            self.build_model(
                load_pretrained=True if self.weight_path else False)
    ...
    def build_model(self, load_pretrained=True):
        ...
        y_true = [
            layers.Input(name='input_2', shape=(int(
                52*(size/416)), int(52*(size/416)), 3, (self.num_classes + 5))),  # label small boxes
            layers.Input(name='input_3', shape=(int(26*(size/416)), int(26 * \
                         (size/416)), 3, (self.num_classes + 5))),  # label medium boxes
            layers.Input(name='input_4', shape=(int(
                13*(size/416)), int(13*(size/416)), 3, (self.num_classes + 5))),  # label large boxes
            layers.Input(name='input_5', shape=(
                self.max_boxes, 4)),  # true bboxes
        ]
    ...
    def export_prediction(self, annotation_path, pred_folder_path, img_folder_path, bs=2):
        with open(annotation_path) as file:
            ...
            for batch_idx in tqdm(range(0, len(img_paths), bs)):
                ...
                b_boxes, b_scores, b_classes, b_valid_detections = self.inference_model.predict(
                    imgs)
                for k in range(len(paths)):
                    ...
                    # print(raw_img_shape)
                    boxes[:, [0, 2]] = (
                        boxes[:, [0, 2]] * raw_img_shape[1])  # w
                    boxes[:, [1, 3]] = (
                        boxes[:, [1, 3]] * raw_img_shape[0])  # h
                    ...
                    with open(output_path, 'w') as pred_file:
                        for box_idx in range(num_boxes):
                            ...
                            if (class_id == 0 or class_id == 2):
                                pred_file.write(
                                    f'{class_id} {scores[box_idx]} {b[0]} {b[1]} {b[2]} {b[3]}\n')
    ...
    def eval_map(self, gt_folder_path, pred_folder_path, temp_json_folder_path, output_files_path):
        ...
        for txt_file in ground_truth_files_list:
            ...
            for line in lines_list:
                ...
                # ONLY GET PERSON AND VEHICLE OBJECT CLASSES
        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            bounding_boxes.sort(key=lambda x: float(
        ...
        with open(output_files_path + "/output.txt", 'w') as output_file:
            ...
            print(total_tps, total_fps, mAP)

            # with open(ensembles_output_path, 'w') as ensemble_output_file:
            #  ensemble_output_file.write()
    ...
    def predict_nonms(self, img_path, iou_threshold=0.413, score_threshold=0.1):
        ...
        output = yolov4_head(yolov4_output, self.num_classes,
                             self.anchors, self.xyscale)
        pred_output = nms(output, self.img_size,
                          self.num_classes, iou_threshold, score_threshold)
        pred_output = [p.numpy() for p in pred_output]
        detections = get_detection_data(img=raw_img,
                                        model_outputs=pred_output,
                                        class_names=self.class_names)
        draw_bbox(raw_img, detections,
                  cmap=self.class_color, random_color=True)
        return detections
    ...
    def custom_non_max_suppression(self, predictions, iou_threshold=0.5):
        if (len(predictions) == 0):
            return False

        rows, columns = predictions.shape

        sort_index = np.flip(predictions[:, 1].argsort())
        predictions = predictions[sort_index]

        boxes = predictions[:, 2:6]
        # print('boxes', boxes)
    ...
    def custom_nms(self, pred_result, pred_result_TTA, pred_result_nms):
        baseline_files_list = glob(pred_result + '/*.txt')
        assert len(baseline_files_list) > 0, 'no baseline file'
        baseline_files_list.sort()

        for txt_file in baseline_files_list:
            file_id = txt_file.split(".txt", 1)[0]
            file_id = os.path.basename(os.path.normpath(file_id))
            ...
            for line in lines_list:
                ...
                bbox = [float(class_name), float(conf), float(
                    left), float(top), float(right), float(bottom)]
                # To filter out faults that produce boxes with area = 0
                area_aux = (float(right) - float(left)) * \
                    (float(bottom) - float(top))
                if (area_aux > 0):
                    bboxes.append(bbox)

            temp_path = os.path.join(pred_result_TTA, (file_id + ".txt"))
            assert os.path.exists(
                temp_path), "Error. File not found: {}\n".format(temp_path)
            lines_list_2 = read_txt_to_list(temp_path)
            ...
            bboxes_nms_indices = self.custom_non_max_suppression(bboxes)

            output_path = os.path.join(pred_result_nms, (file_id + ".txt"))
            with open(output_path, 'w') as pred_file:
                for idx_bbox, bbox_final in enumerate(bboxes):
                    if (bboxes_nms_indices[idx_bbox]):
                        class_value = bbox_final[0]
                        scores = bbox_final[1]
                        x1 = bbox_final[2]
                        y1 = bbox_final[3]
                        x2 = bbox_final[4]
                        y2 = bbox_final[5]
                        pred_file.write(
                            f'{int(class_value)} {scores} {x1} {y1} {x2} {y2}\n')
