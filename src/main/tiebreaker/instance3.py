...
class Receiver(Node):
    ...
    def listener_callback(self, msg):
        ...
        if not msg.data:
            ...
            tiebreaker_future = self.tiebreaker_client.cli.call_async(
                self.tiebreaker_client.req)
            rclpy.spin_until_future_complete(
                self.tiebreaker_client, tiebreaker_future)
            time.sleep(1)
            sys.exit()
        ...
        if ret:
            ...
            for box, score, cl in zip(boxes, scores, classes):
                ...
                self.tiebreaker_client.req.x_coord.append((x_min + x_max)/2.0)
                self.tiebreaker_client.req.y_coord.append((y_min + y_max)/2.0)
                self.tiebreaker_client.req.classes.append(name)
                self.tiebreaker_client.req.classes_confidence.append(
