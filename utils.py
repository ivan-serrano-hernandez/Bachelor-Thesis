...
def load_weights_unfrozen_layers(model, weights_file_path):
    conv_layer_size = 110
    conv_output_idxs = [93, 101, 109]
    with open(weights_file_path, 'rb') as file:
        major, minor, revision, seen, _ = np.fromfile(
        ...
        for conv_idx in range(conv_layer_size):
            ...
            if conv_idx not in conv_output_idxs:
                if conv_layer.trainable == False:
                    conv_layer.set_weights([conv_weights])
                if bn_layer.trainable == False:
                    bn_layer.set_weights(bn_weights)
            else:
                if conv_layer.trainable == False:
                    conv_layer.set_weights([conv_weights, conv_bias])
...
def get_detection_data(img, model_outputs, class_names):
    ...
    boxes, scores, classes = [output[0][:num_bboxes]
...
def nms_output_to_bboxs(model, b_boxes, b_scores, b_classes, b_valid_detections, img):
...
def draw_bbox(img, detections, cmap, random_color=True, figsize=(10, 10), show_img=True, show_text=True):
    ...
    for _, row in detections.iterrows():
        ...
        color = list(np.random.random(size=3) *
                     255) if random_color else cmap[cls]
        ...
        if show_text:
            ...
            cv2.putText(img, text, (x1, y1), font, font_scale,
...
class DataGenerator(Sequence):
    ...
    def __init__(self,
        ...
        self.num_classes = len([line.strip()
    ...
    def __data_generation(self, annotation_lines):
        ...
        X = np.empty((len(annotation_lines), *
                     self.target_img_size), dtype=np.float32)
        y_bbox = np.empty(
            (len(annotation_lines), self.max_boxes, 5), dtype=np.float32)  # x1y1x2y2
...
def preprocess_true_boxes(true_boxes, input_shape, anchors, num_classes):
    ...
    true_boxes_xy = (true_boxes_abs[..., 0:2] +
                     true_boxes_abs[..., 2:4]) // 2  # (100, 2)
    true_boxes_wh = true_boxes_abs[..., 2:4] - \
        true_boxes_abs[..., 0:2]  # (100, 2)
    ...
    grid_sizes = [input_shape//{0: 8, 1: 16, 2: 32}[stage]
    ...
    for batch_idx in range(bs):
        ...
        iou = intersect_area / (box_area + anchor_area -
                                intersect_area)  # (# of bbox, 9)
        ...
        for box_idx in range(num_boxes):
            ...
            for stage in range(num_stages):
                if best_anchor in anchor_mask[stage]:
                    x_offset = true_boxes[batch_idx,
                                          box_idx, 0]*grid_sizes[stage][1]
                    y_offset = true_boxes[batch_idx,
                                          box_idx, 1]*grid_sizes[stage][0]
                    # Grid Index
                    ...
                    class_idx = true_boxes[batch_idx,
                                           box_idx, 4].astype('int32')
                    ...
                    y_true[stage][batch_idx, grid_row, grid_col, anchor_idx,
                                  :2] = true_boxes_xy[batch_idx, box_idx, :]  # abs xy
                    y_true[stage][batch_idx, grid_row, grid_col, anchor_idx,
                                  2:4] = true_boxes_wh[batch_idx, box_idx, :]  # abs wh
                    y_true[stage][batch_idx, grid_row, grid_col,
                                  anchor_idx, 4] = 1  # confidence

                    y_true[stage][batch_idx, grid_row, grid_col,
                                  anchor_idx, 5+class_idx] = 1  # one-hot encoding
                    # smooth
